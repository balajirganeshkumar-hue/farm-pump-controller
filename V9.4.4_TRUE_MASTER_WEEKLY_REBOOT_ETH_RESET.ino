...
void ethernetSupervisor(){
  ...
  if(linkNow && !ethLinkUp){
    ...
    byte mac2[] = {0x02, 0x32, 0x45, 0x56, 0x78, 0x9A};
...
void setup(){
  ...
  Ethernet.init(5);
  byte mac[] = {0x02, 0x32, 0x45, 0x56, 0x78, 0x9A};
...
/**************************************************************
 * V9.4.4 ADDITIONS
 * ✅ Weekly reboot every 7 days (only if pump OFF)
 * ✅ W5500 Reset Pin GPIO25
 *************************************************************/

#define W5500_RESET_PIN 25
...
unsigned long lastWeeklyReboot = 0;

void weeklySafeRebootCheck() {
  if (!pumpRunning) {
    if ((millis() - lastWeeklyReboot) > (7UL*24UL*60UL*60UL*1000UL)) {
      lastWeeklyReboot = millis();
      client.publish("farm/pump/status", "WEEKLY_REBOOT");
      delay(3000);
      ESP.restart();
    }
  }
